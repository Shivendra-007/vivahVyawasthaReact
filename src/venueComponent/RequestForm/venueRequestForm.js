...
import venueApi from '../../webApi/Venue/venue-api';
...
export default function RequestForm({ open, setOpen, venueDetailsId }) {
  ...
  const addRequest = async (event) => {
    try {
      ...
      let response = await axios.post(baseUrl.BASE_URl + venueApi.ADD_Request, { contactPerson, contactNumber, totalGuest, typeOfEvent, checkIn, checkOut, customerId,venueDetailsId})
      if(response.status){
       setOpen(false);
