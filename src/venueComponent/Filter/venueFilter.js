...
function VenueFilter({ GetVenue, setVenueList, setIsLoading, setError }) {
  ...
  const getVenueCategory = async () => {
    try {
      var response = await axios.get(baseUrl.BASE_URl + api.GETVENUE_CATEGORY);
      if (response.status) {
  ...
  const byCategory = async (categoryId) => {
    try {
      setIsLoading(true)
        var response = await axios.post(baseUrl.BASE_URl + api.GETVENUE_BY_CATEGORY, { categoryId: categoryId });
        if (response.status) {
          setVenueList(response.data.venueList)
          setIsLoading(false);
        }
  ...
  const byCapacity = async (first,second) => {
    try {
      setIsLoading(true)
        var response = await axios.post(baseUrl.BASE_URl + api.GETVENUE_BY_CAPACITY, { first:first,second:second});
        if (response.status) {
          setVenueList(response.data.venueList)
          setIsLoading(false);
      }
  ...
  const byCharges = async (first,second) => {
    try {
      setIsLoading(true)
        var response = await axios.post(baseUrl.BASE_URl + api.GETVENUE_BY_CHARGES, { firstPrice:first,secondPrice:second});
        if (response.status) {
          setVenueList(response.data.venueList)
          setIsLoading(false);
      }
